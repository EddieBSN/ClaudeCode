...
async function shoot(browser, { width, dsf, out }) {
  ...
  // Expand the viewport to the full content height and capture in a single shot.
  // This avoids the fullPage tiling path, which mis-composites the 3D-transformed
  // artifact layer. It also brings every reveal element into view so the
